...
export function uncarriedWordOf(method: string, path: string): string {
  ...
  // `board`, `board.items` and `timeline` were here until this console began
  // asking for them. What is carried is parsed once, by the reader's own case,
  // and a second spelling of it here would be a second thing to keep right —
  // the same reason the writer's routes were never spelled twice. A POST to
  // `/v1/board` is an item write, which this daemon refuses by name at the
  // route, so it is not a word this table has to name either.
