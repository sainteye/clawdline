...
export function boardResponsibility(item, zh = false) {
    const row = item && item.responsibility || {}, status = row.status;
    const copy = {
        live_session: ["Responsible Session: " + (row.title || item.owner || "Session") + " · live",
            "負責 Session：" + (row.title || item.owner || "Session") + "・仍在線"],
        session_not_live: ["Recorded Session is no longer live · needs takeover",
            "原負責 Session 已不在線・需要接手"],
        session_ambiguous: ["Responsible Session identity is ambiguous",
            "負責 Session 身分有衝突"],
        inventory_unknown: ["Session liveness is not established",
            "Session 是否在線目前無法確認"],
        unassigned: ["No responsible owner", "尚未指派負責人"],
        recorded_owner: ["Responsible: " + (item.owner || "unknown"),
            "負責人：" + (item.owner || "未知")]
    }[status];
    return copy ? { status, text: copy[zh ? 1 : 0], live: row.live } : null;
}
...
// A first screen, and what "show more" adds to it.
const FIRST_PAGE = 30, PAGE_STEP = 30;
...
function card(ctx, item) {
    ...
    const responsibility = boardResponsibility(item,
        /^zh/i.test(ctx.doc.documentElement.lang || ""));
    if (responsibility) {
        const owner = el(ctx, foot, "span", responsibility.text,
            "board-responsibility board-responsibility-" + responsibility.status);
        owner.dataset.boardResponsibility = responsibility.status;
    }
...
function renderSessionOwnership(ctx, parent, item) {
    ...
    const responsibility = boardResponsibility(item,
        /^zh/i.test(ctx.doc.documentElement.lang || ""));
    if (responsibility)
        fact(ctx, part, words(ctx, "Current responsibility", "目前責任狀態"),
            responsibility.text);
...
export function bindBoardPage(elements, environment = {}) {
    ...
    const state = (ctx.state = {
        ...
        items: [],
        audience: null,
        // How many rows the next list read asks for, and how many the Project has.
        //
        // A first paint is a screen, not a catalogue. The complete human list for the busiest
        // Project measured 223,373 bytes on 2026-09-16 — 94 cards the page draws all of because it
        // renders every group — against a 150 KB budget for a first paint. Thirty is what somebody
        // can see; the rest arrives when they ask for it.
        //
        // The whole prefix is re-read rather than merged, and that is the simpler correctness:
        // "show me thirty more" is `limit += 30`, one request, no accumulated list to keep in step
        // with a poll that is also rewriting it. At a page of thirty and a Project of ninety-four
        // that is at most three reads, and the poll below asks for exactly what is already on
        // screen so nothing a reader is looking at goes stale or disappears.
        pageLimit: 0,
        pageTotal: null,
        item: null,
        projectId: null,
    ...
    function renderBody() {
        ...
        // The rest of the list, when a first screen is not all of it.
        //
        // Not shown while searching: a search is already answered across the whole catalogue by
        // the button above, and two "load more" controls meaning different things would be one
        // control too many. The count is what is left rather than what is here, because "還有 37
        // 筆" answers the question somebody is asking when they reach the bottom.
        if (!query && state.audience && Number.isSafeInteger(state.pageTotal)
            && humanAll.length < state.pageTotal) {
            const remaining = state.pageTotal - humanAll.length;
            button(ctx, target,
                words(ctx, `Show more (${remaining} left)`, `顯示更多（還有 ${remaining} 筆）`),
                "board-items-more",
                () => {
                    state.pageLimit = (state.pageLimit || FIRST_PAGE) + PAGE_STEP;
                    load();
                },
                "board-button board-load-more");
        }
    ...
    function load(quiet = false) {
        ...
        const promise = Promise.resolve()
            .then(() => {
                ...
                // A project's list is read for one audience. The page draws only human rows unless
                // somebody is searching — both the agent and the archive sections are behind
                // `if (query && …)` below — and of 481 cards in the busiest Project on 2026-09-16,
                // 94 were human. What the other 387 cost was a megabyte on every fifteen-second
                // poll, for rows nobody had opened a section for.
                if (project && !item && environment.readItems) {
                    // `pageLimit` is what is on screen, so a poll refreshes exactly that and a
                    // reader never watches a row they were looking at vanish.
                    return environment.readItems(project, "human", 0,
                                                 state.pageLimit || FIRST_PAGE,
            ...
            .then((answer) => {
                ...
                // Set when the answer deliberately holds one audience. It is not truncation — the
                // human list is complete — but it does mean the rows in memory are not every row
                // this Project has, so a search must reach the server rather than filter what is
                // here. Keeping the two apart matters: `truncated` also writes "some records are
                // not loaded" into the status line, and that would be false.
                state.audience = typeof board.audience === "string" ? board.audience : null;
                if (state.audience) {
                    if (!state.pageLimit) state.pageLimit = FIRST_PAGE;
                    const page = board.page;
                    state.pageTotal = page && Number.isSafeInteger(page.total) ? page.total : null;
                }
                ...
                // A paged answer is partial on purpose, and the page says so twice already — the
                // "loaded 30 / 94" line and the button under it. Repeating it here as a warning
                // would tell a reader something is wrong with a view that is working exactly as
                // designed. The sentence is kept for the truncation it was written for: a
                // materialization that dropped rows nobody can ask for, which carries no `page`.
                if (board.truncated && !board.page)
    ...
    function searchCatalog(rawQuery, offset = 0) {
        ...
        if (!query || !project || (!state.truncated && !state.audience)
    ...
    function open(project, item, presentation, machine, routeError, locatorBacked = false) {
        ...
        state.readStatus = "loading";
        // A different Project starts at a first screen again. The same one keeps what the reader
        // has already asked to see, so a refresh or a route change does not collapse the list
        // under them. This reads `previousProject`, captured at the top, because the line below
        // has not run yet — comparing against `state.projectId` after it would compare a value
        // with itself and never reset.
        if (previousProject !== (project || null)) { state.pageLimit = 0; state.pageTotal = null; }
