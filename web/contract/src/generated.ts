 ...
 * refused as input_moved, input_behind_question or input_unreadable with nothing
 * pressed otherwise. Anything else is refused before the session is looked up.
 ...
 * — project_dir, claims and the rest — are set in the file and carried across
 * every save. permission_mode is the form's own field (ScheduleRequest).
 */
...
/**
 * The tail of one command's output file. A reader repaints only when `signature`
 * changes and stops asking once `ended` is true.
 */
export interface ShellOutputReply {
  /**
  ...
  /**
   * The command's row, as the session list carries it. `doing` is absent once it
   * has ended.
   */
  shell: SessionShell
