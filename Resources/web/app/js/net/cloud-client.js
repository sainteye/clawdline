...
export class CloudClient {
    ...
    async start(options) {
        ...
        ws.onclose = function (event) {
            ...
            var relayCode = (self.lastRelayError && self.lastRelayError.code) || closeCodeName(closeCode);
    ...
    diagnosticsReport(report, machine) {
        try {
            ...
            return this._machineRequest(machine || this._onlyMachine("diagnostics"),
    ...
    _readTimedOut(key, waiters, probe) {
        ...
        this._readCloudStatus(waiters.machine, this.statusProbeTimeoutMs).then(function (status) {
            ...
            self._settleRead(key, null, failure);
        });
    }
    ...
    _failAllReads(error) {
        ...
        Array.from(this.pendingBySequence.entries()).forEach(function (entry) {
            if (entry[1].ack) self._failPending(entry[0], error);
        });
    }
    ...
    answer(value, answer) {
        ...
        if (this.macCapabilities.has(identity.machine)) body.request = requestID();
