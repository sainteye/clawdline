...
標題時發一則進度回報。這樣 child 死在第三個小時只損失一小時——branch 上還留著其餘的。
...
量測）。「不弱於被審者」會允許用 haiku 去審 haiku 寫的葉節點，而審查的價值完全等於審查者判斷力的
價值：漏掉的發現會一路帶到最後。
...
**指令裡要求它在頭三分鐘發一則進度回報**（**不要指定管道**：codex 完全沒有網路，
`CODEX_SANDBOX_NETWORK_DISABLED=1`，連 `127.0.0.1` 都是 exit 7、DNS 也關著，所以它寫
`progress.json` 到自己的任務目錄、由 broker 像撿 `result.json` 一樣撿走；claude 兩種都可以。
每個 child 自己的 briefing 會帶對它有效的那一種），講它讀完 briefing 之後決定要怎麼做——在開工前，
...
6. **恢復，並且跟每條線要「內部盤點」和「給人的收尾」。** 在任何東西重新開始之前，分開問每一條線：
   ...
   **🚀 上線狀態**、**🔒 這個 Session 可以關閉嗎？**。「沒有」也是完整答案，省略不是。它必須分清楚
   ...
   在 ID 還不能點擊導覽以前，絕不能把它當成人類摘要的主要名稱。「讓摘要裡的 Session 引用可以直接
   點開」要登記成產品待辦，不能反過來要求使用者拿 ID 在分頁裡找。
...
**還有，`OBJECTIVE` 結尾要放那一句「請接手方替自己命名」。** 寫進文件裡——不是寫在這裡，也不是
寫在附帶的訊息裡。接收端只會被一句話碰到，而那句話只指向 `handoff.md`、不指向別的；接手的那個
session 通常還在別的專案裡，那裡沒有裝這份 skill：只寫在這個檔案裡的指示，等於寄件方讀得到、
接收端永遠看不到。那一句，照這樣寫：

> 開工前先替這個 session 命名，名字就是這件工作：`POST /v1/sessions/<你自己的 session id>/title`，
> body 是 `{"title":"<這件工作>"}`，帶 `Authorization: Bearer $(cat ~/.config/clawdline/remote-token)`
> 和一個新的 `Idempotency-Key`。**這通呼叫穩定買到的是本機名字**，它排在 Clawdline 給這個分頁的
> 標籤之上。它也會試著往下游同步成助理自己的名字，但**在自己的回合裡呼叫它的 session 就是
> working**，所以預期會拿到 `downstream: "busy"`——而 busy 是刻意不排隊的，不要用重試迴圈去磨它。
> 之後真人自己下的 `/rename` 還是贏。

**還有第二句，放在回合的另一端。** 決定它的是同一個理由：接手方是在一個沒有裝這份 guide 的專案裡
收工的，所以只寫在這裡的指示它永遠看不到。2026-09-05 真的發生了——某個接手方照文件在開工時打了
命名那一通，收工時什麼都沒回報；事後問它，它說它去找過發佈用的路由，結論是 root 根本沒有這種管道。
同一類的兩通呼叫，一通寫進了包裹、一通沒有。照這樣寫：

> 這一輪真的做完時——含整合、驗證與必要的 commit——送一筆**交付回報**，那就是讓這個 session 在
...
這兩句給的都是路由不是指路，而這正是整個區塊在做的取捨：對一個碰不到指路目標的 session 來說，
指路一文不值；而路由會對不上比較新的 build。接手方**確實有** skill 的時候優先給指路，並在文件裡
說明你給的是哪一種。

Clawdline 本來就會用你給的 `title` 標記它開出來的那個分頁，而且現在重開 app 也還記得——所以這步
不是補救，是**觸及範圍**；而且**這是接收端的動作、不是你的**：你沒有它的 session id，而它自己打的
