...
- **同一個問題，在手機上也回答。** 你的 Mac 開一個網頁，手機打開它，就讀得到每個 session
  在做什麼，逐字稿也在裡面——第二個開關打開之後還可以打字進去。全新安裝是關著的、只綁 loopback、
  每一個請求都要帶裝置 token，而配對用的那組數字**只出現在 Mac 上**。
  要從外面連進來靠 `cloudflared`，那是你自己裝的程式，不是這個 app 帶來的。
  有 session 開始等你的時候，手機會震一下。
  → [用瀏覽器，或用手機](#用瀏覽器或用手機)

  <img src="docs/assets/web.gif" width="390" alt="手機上的那個網頁：六個 session、各自帶著專案的圖示，
  在問你話的那個用重點色挑出來。點進去看到那個問題，還有一行說選單得在 Mac 上回答。退回清單、
  換另一個 session 的逐字稿，兩個工具呼叫摺成一行，點開才展開；最後在下面的框打一句話送出去">

- **瀏海裡也會講。** 你的吉祥物住在鏡頭那塊：什麼都沒在跑的時候牠在那裡睡覺，有東西在跑就探出來，
  有人在等你就講出是哪一個，長工作跑完就跳舞。**看起來有多忙，就是你手上有多少東西在跑。**
  設定裡一個字就能整個關掉。
  → [瀏海](#瀏海)

  <img src="docs/assets/island.gif" width="760" alt="選單列，還有切進去的那個瀏海：一個 session 在跑的時候吉祥物從鏡頭那塊探出來，變成三個的時候旁邊多一個數字，接著形狀往右邊長出去、用 accent 色講出在等你的是哪一個——最後長工作跑完，一個綠點、跑完的是哪一個，還有吉祥物在跳舞">
...
- **人不在 Mac 前面** — [手機打開的那個網頁](#用瀏覽器或用手機) · [中間擋著哪些東西](#中間擋著哪些東西) · [tunnel](#手機不在同一個房間的時候tunnel) · [通知](#讓它講而不是自己去看)
...
<div align="center">
<img src="docs/assets/sessions.gif" width="820" alt="五個 session 的狀態在你眼前換：一個在跑、帶著 Claude Code 自己畫的那行字，一個用 accent 色說在等你回答，三個安靜——接著等你的那個被回答了於是安靜下來、跑很久的那個跑完、本來什麼都沒說的那個開始發問。每一行都帶著自己專案的像素圖示，下面那一行則是選取走到哪一個就寫哪一個">
</div>
...
**你的 Mac 開一個網頁，你的手機打開它。** 每一個 session 的狀態、它所屬專案的圖示、
以及照 <kbd>⌘</kbd><kbd>J</kbd> 那樣排版過的逐字稿；第二個開關打開之後，還有一個框可以打字進去。
桌機是兩欄、配上 app 自己那套快速鍵；手機是單欄，設計上就是要被加到主畫面的——
加了之後它的行為像 app，不像網頁。它跟輸入條講同樣的十四種語言，
而且 `lang` 是 `auto` 的時候，它用的是**手機**的語言，不是 Mac 的——因為拿在手上的是手機。

<img src="docs/assets/web-wide.png" width="860" alt="同一個網頁在筆電上：左邊是 session 清單、
在等你的那個用重點色標出來，右邊是它的逐字稿——標題、有框線的表格、一段話——下面是打字的框，
最底下是 app 自己那套快速鍵">

**全新安裝是關著的**，要你自己去打開。這不是誰隨手選的預設值：
一個在聽的 socket，是「你機器上的一支程式」跟「你機器上的一個服務」的差別，
而這個差別應該是你刻意做的事。
...
- **它只綁 `127.0.0.1`，沒有別的。** 不是「全綁然後過濾」——這個 listener 建立時就要求
  local endpoint 是 loopback，所以你的網路上根本沒有一個介面可以找到它。
  出去的路是一條**往外撥**的 tunnel，不是一個坐在那裡等的 port。
- **`Host` 在任何東西被看之前先被檢查。** `evil.com` 上的一個網頁本來就能 `fetch`
  `http://127.0.0.1:7717/…`；救了一般本機伺服器一命的是它**讀不到**回應，因為 origin 不同——
  而 DNS rebinding 就是專門拿掉這一層的。rebinding 唯一改不掉的東西是 `Host`，它還是寫著
  `evil.com`，所以只要 `Host` 不是這台伺服器認得的名字，請求當場就被擋掉。
  它認的是 `127.0.0.1`、`localhost`、`::1`、`remote_hostname` 填的那個，以及
  `.trycloudflare.com` 底下的任何名字。
- **跨站請求一律不回。** 依據是 `Sec-Fetch-Site`——瀏覽器自己設、網頁偽造不了的那個標頭。
  「人點連結進來」放行，因為在別的頁面上把網址打進網址列也算跨站；分得開它們的是 mode，不是 site。
  會改東西的請求還要再過一次 `Origin`，因為 cookie 是瀏覽器自己送的，不管發請求的網頁想不想要。
- **其餘每一條路都要有裝置 token**，不管請求從哪裡來。256 個隨機位元，只存 SHA-256、
  用固定時間比對——包含查表那一步，所以錯的 token 問不出哪些 device id 是真的。
  不用 token 的只有：網頁本身、它的圖示與 manifest、`/v1/health`、介面自己的文案，以及配對那幾條。
  **loopback 沒有例外**，而且這不是潔癖：tunnel 一開，從別的國家用手機發的請求
  也是從 `127.0.0.1` 進來的，所以「可是它是本機來的」正好會在它本來要防的那個情況下失效。
- **配對靠一組只出現在 Mac 上的數字。** 瀏覽器提出要求，拿回一個 id；那六位數字出現在
  **Mac 螢幕上**的一個提示框裡，從來不在那個回應裡。五次機會、兩分鐘、同一時間只有一組、
  十分鐘內最多三次。誰都可以發起配對，只有看得到你螢幕的人完成得了。
- **打字進 session 跟讀 session 是兩個開關。** 讀會交出 repo 名稱、分支、任務標題；
  寫則是遠端執行程式碼，因為 Claude Code 會跑 `bash`。這是兩種風險等級的兩個功能，
  所以是兩個開關，不是同一個刻度上的兩格——而且送字是**整批**授予配對裝置的，
  要收回的時候就是一次收回全部。
- **在配對過任何裝置之前，tunnel 拒絕啟動。**「從網際網路可達」應該是一個人做的決定，
  不是改一個設定鍵就會發生的事。
- **做過什麼會被寫下來。** 每一次配對、撤銷、送字、開新 session 都進
  `~/.config/clawdline/remote-audit.jsonl`，權限 `0600`，只追加不改寫——
  因為萬一真的有人進來了，你要問的是「他做了什麼」，而那個問題事後補不出答案。

**配對過的裝置在你撤銷之前一直是信任的。** 沒有期限、也不會再問一次：它手上那把 token
一直有效，直到你在設定 → 遠端 把它拿掉——連同它的通知一起。
[docs/remote.md](docs/remote.md) 裡是誠實的另一半：走 tunnel 就代表 TLS 在 Cloudflare 邊緣
解開，你的逐字稿以明文經過那裡；quick tunnel 的網址本身就是一個憑證；
而這一層對「已經用你的身分在你 Mac 上跑著的東西」完全不設防。

### 怎麼打開

**設定 → 遠端 → 讓瀏覽器或你的手機看得到你的 session。** 如果瀏覽器就在這台 Mac 上，這樣就完了：
按**用瀏覽器打開**，它會發一把只屬於那個瀏覽器的鑰匙，然後開一個已經登入的頁面。

**手機的話，按「配對手機……」會畫一個 QR code。** 那張圖裡是一把為這次掃描現發的鑰匙，
所以有人拍下你的螢幕，拍到的是一個你在清單裡看得到、也拿得掉的裝置，不是這台 Mac 自己的鑰匙。
另一條路是從手機那邊輸入六位數字——可以，代價是走一趟機器前面。

### 手機不在同一個房間的時候：tunnel

手機連不到 `127.0.0.1`，所以只要有公開網址，QR code 就會指向公開網址。
那個網址來自 `cloudflared`：它**往外**撥給 Cloudflare，流量再沿著它撥出去的那條連線回來——
沒有 port 要轉發，你的網路上也沒有任何東西在聽。

**`cloudflared` 是你自己裝的程式。** 這個 app 不打包它、也不會替你下載；
`dependencies: none` 之所以還是真的，就是因為它是**你的** binary——跟 `tmux`、`whisper-cli`
一樣，去套件管理器放它的地方找。`brew install cloudflared`，或把路徑填進 `cloudflared_path`。
兩種模式：

- **`quick`** 不需要 Cloudflare 帳號。它每次跑都生一個新網址——`trycloudflare.com`
  底下四個英文單字——這一端是從 cloudflared 自己的 log 讀回來的。適合用一個下午。
  網址每次都變，而且在那一輪裡它就是憑證：拿到的人就到得了你的登入頁。
- **`named`** 跑的是你自己用 `cloudflared tunnel create` 建的 tunnel，配上你指過去的網域。
  `remote_tunnel_name` 跟 `remote_hostname` 兩個都要填，少一個它會講出來而不是硬開——
  因為 named tunnel 的網址 cloudflared 從來不印，這一端算不出來，
  硬開的結果是一條起得來、但沒有人講得出網址的隧道。

Clawdline 會自己寫一份 `~/.config/clawdline/cloudflared.yml` 並叫 cloudflared 讀那一份，
所以你的 `~/.cloudflared/config.yml` 不會被拿來用在這件事上。這不是為了整齊：
那個檔案的 `tunnel:` 會**蓋掉命令列上的名字**，而它的 ingress 清單會很客氣地對一個
它沒聽過的網域回 `404`。

### 讓它講，而不是自己去看

配對過的手機可以訂通知，之後**有 session 開始等你回答**的時候它會震一下——
那是唯一一種每過一秒都在賠錢的狀態。另外兩種預設關著、要自己開：
`push_on_finish` 是「跑超過兩分鐘的一輪停了」，`push_on_deploy` 是「deploy 不再跑了」，
成功失敗都送。

<img src="docs/assets/web-push.gif" width="390" alt="通知落到手機上：橫幅從上面滑下來、帶著 app
自己的圖示，停在那裡夠你讀完，然後收回去。這一則是網頁自己要來的測試通知；沒叫它送就自己來的
那些，講的是哪一個專案在等你">

訊息是對那個裝置加密封起來的，所以推播服務手上只有一團密文，只知道「有東西送到某個訂閱」。
但加密決定的是「路上誰讀得到」，決定不了「它亮在桌上那支鎖著的手機上時誰讀得到」——
所以裡面放的是專案跟狀態，永遠不是任務內容。

**Mac 必須是醒著的。** 這不是一個跑在別處的服務，是你的機器醒著、看到了、然後送出去。
睡著、關機、或 app 關掉，就什麼都不會送，也不會存起來之後補送。
另外在 iOS 上，那個頁面得先加到主畫面、再從那裡打開——Apple 只把通知送給住在那裡的 web app，
那是他們的規則，不是這裡的設定。

### 從沙發上開一個新的

送字打開之後，配對過的裝置還可以**開一個新的 Claude Code session**，位置是這台 Mac
待過的目錄。**client 從來不送路徑**：它送的是一個不透明的 id，對應到 Mac 自己從
Claude Code 的紀錄整理出來的清單，而指令是寫死的 `claude`、不帶任何參數。
那條路上根本沒有一個欄位可以寫進目錄或指令——這比「路徑有驗證」強，
因為驗證是下一個改這個檔案的人可能不小心弄鬆的東西，而「參數不存在」不是。
它需要 iTerm2 開著、或者有 tmux 可用；而且**不會搶焦點**：人在 Mac 前面的話，人正在忙別的。
在網頁上，它是篩選框旁邊那顆 `+`：Mac 自己整理出來的清單，連圖示一起；
而分頁已經開了、session 還沒出現的那段時間，標題列下面會有一行字說它在等。

**一個現在還粗的地方。** 已經打開的頁面會停在它載入時的那份介面，
所以 app 更新之後，你得自己重新整理一次。

`docs/remote.md` 裡是完整的威脅模型，包含**這一層防不到什麼**；
...
<div align="center">
<img src="docs/assets/island.gif" width="820" alt="一條選單列，還有切進去的那個瀏海，就這樣而已：形狀從那個洞往兩邊長出來——先是一個 session 在跑、只有吉祥物，然後變成三個、旁邊多一個數字，有人在等你的時候是任務名字加一個 accent 色的點，最後長工作跑完，一個綠點加上吉祥物跳舞">
</div>

你的吉祥物住在鏡頭旁邊那條選單列裡。有東西在跑就探出來——**牠看起來有多忙，就是你手上有多少東西在跑**
——有人在等你就講出是哪一個，長工作跑完就跳舞。

**什麼都沒在跑的時候，牠還是在那裡，只是睡著了。** 只有那隻角色，閉著眼睛慢慢呼吸：沒有耳朵、
沒有名字、沒有數字。你的機器一天之中大部分時間都是這個狀態，所以它是設計來讓你忘記它的存在、
而不是讓你去讀它——有工作進來的時候牠會伸個懶腰醒過來，然後開始做事。
...
而且在那裡它不會睡。一個膠囊掛在那邊撐過一份工作的那幾分鐘沒問題，整天杵在你的選單列上就是另一回事，
所以沒有鏡頭挖孔的螢幕行為跟以前完全一樣：有話要講的時候出現，講完就收回去。
...
> 腳要放在最下面那一列才站得住。動畫都要寫：`pop`、`idle`、`typing`、`dance`、`cheer`、`stretch`，
> 還有 `sleep`——一段長的、會循環的、閉著眼睛的呼吸，不要 blink 區塊，給瀏海在什麼都沒跑的時候用。
...
完整格式、每一段動畫的觸發時機、以及這個尺寸下什麼畫得出來：**[docs/mascots.md](docs/mascots.md)**。
...
```jsonc
{
  ...
  "remote": false,                       // 開網頁介面——預設關著，要自己打開
  "remote_port": 7717,                   // 只綁 loopback；能不能從外面連是隧道決定的
  "remote_write": false,                 // 配對過的裝置能不能打字進 session，還是只能讀
...
**這裡有兩件事會連網，而且兩件都是你自己扳下去的開關。**
一件是[遠端存取](#用瀏覽器或用手機)：全新安裝關著，打開之後也只綁 loopback，
真正把東西帶出這台機器的是 `cloudflared`——你自己裝的那支。送到手機的通知是同一個開關的另一半，
而且是加密封起來送出去的。另一件是語音輸入，就是下面這段。

**語音輸入在連網的時候會講。** macOS 對「你下載過的聽寫語言」
在本機辨識，沒下載的則把聲音送到 Apple。現在是哪一種，會整段寫在輸入條下緣——
**麥克風不會在那行字沒出現的情況下開著**，收起面板也會停止。想要它永遠不出這台機器，
到系統設定 › 鍵盤 › 聽寫把那個語言下載下來。

其他部分都不連網——遠端存取關著、又沒去按麥克風的話，就是完全不連。
歷史紀錄存在 `~/.config/clawdline/config.json`，不會去任何地方。
