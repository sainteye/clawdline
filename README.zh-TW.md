...
- **在你視線落點打字。** 一條在眼睛高度的輸入條，而不是滿版終端機的左下角。
- **把那個 session 讀回來**（<kbd>⌘</kbd><kbd>J</kbd>）——是排版過的對話，跑完的工具呼叫收成一行，
  上面還有一行說它現在正在做什麼。
- **講清楚是哪個專案**而不只是哪個任務：repo、分支、未提交數量、進行中的部署、backlog，
  還有它自己的圖示與顏色。
- **檔案與圖片直接丟進來**，畫面上是縮圖，送出去的是路徑。
- **用說的代替打字**，語言有裝的話不出這台機器。
- **換上你自己畫的吉祥物**，不必 fork 任何東西。
...
**可以講一段、動手改字、再繼續講**：輸入框裡任何一處被編輯過，這一輪就結束，
下一句從游標的位置開始寫，而不是插進你剛才正在修的那句話中間。

你下載過的聽寫語言在這台 Mac 上辨識，沒下載的送到 Apple。**現在是哪一種，聽的時候一直寫在畫面上。**
見[權限與隱私](#權限與隱私)。

真的會在一句話裡講兩種語言的話，**[Whisper](docs/whisper.md) 是一個處理得了的選配引擎**——
一行 `brew install` 加一個模型檔，裝好之後 Clawdline 自己就會改用它。代價是文字在你講完之後
才出現，而不是邊講邊出。那一頁有一段可以直接貼給 Claude Code 的 prompt，讓它幫你裝。
