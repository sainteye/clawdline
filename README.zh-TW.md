...
> **[docs/connect.md](docs/connect.md) 就是寫給它看的**——要建哪些檔、格式是什麼、
> 以及怎麼驗自己有沒有做對。它不會安裝任何東西，也不會替你的專案增加任何相依，
> 每一個整合都只是一個 Clawdline 會去讀的小 JSON 檔。
>
...
- **告訴你哪一個 session 在等你。** 四個分頁在做讀起來差不多的任務，在別的工具裡就是四行一樣的字。
  ...
  <img src="docs/assets/sessions-live.gif" width="760" alt="Session 清單，動起來：選取往下走，一個被回答了於是安靜下來、一個跑完、另一個開始發問——上面的分頁列和下面的狀態列也跟著換">
...
- **再往下** — [把你的專案接上來](docs/connect.md) · [用瀏覽器或手機](docs/remote.md) · [API](docs/api.md) · [專案宣告的開發環境](docs/devstack.md) · [Hook：補上那二十秒](docs/hooks.md) · [Whisper 與中英夾雜](docs/whisper.md) · [專案狀態檔](docs/project-status.md) · [吉祥物格式](docs/mascots.md)
...
多數專案有幾個長期跑著的東西——API、前端、worker、tunnel。每一個都是從某個終端機分頁起的，
而那個分頁從此一整天都屬於它。要重啟其中一個，得先找到它的分頁、把全部停掉、再全部起一次。

<kbd>⌘</kbd><kbd>S</kbd> 把它們放到你本來就在打字的地方：
...
起來幾個、跑了多久、每個 port 都是連結、以及它對外的網址。啟動、停止、重啟也都在那裡。

**Clawdline 從來不會自己起一個 process**，而這一條規則就是這件事成立的全部原因。
它起的東西會跟著它一起死——退出時、更新時、當掉時——而一個生命綁在文字輸入框上的
開發環境，比綁在終端機分頁上還糟；至少分頁在它死的時候你看得見。所以它讀你 repo 裡的一個檔案
`.devstack.json`，執行那個檔案指名的指令。process-compose、Overmind、pm2、Docker Compose、
一個用 PID 檔的 Makefile——它們全都可以化約成「一個會印出狀態的指令」，
而這裡沒有任何東西知道你用的是哪一個。

接上去有三個高度，第一個是四行 JSON：

| | | |
|---|---|---|
| **Tier 0** | 宣告哪個 port 屬於哪個服務 | 沒有指令，也沒有東西需要信任 |
| **Tier 1** | 加一個 `status` 指令，印出一份小小的 JSON | 那一列就活了 |
| **Tier 2** | 指向已經在維護狀態的東西——process-compose、Compose | 它讀已經在那裡的東西 |

**在檔案裡指名指令，就代表有東西會去執行它們，所以「把 repo clone 下來」不能構成執行的條件。**
讀宣告出來的 port 不需要任何信任——連一個 TCP port 不會執行任何東西——所以一個未受信任的專案
在 Tier 0 一樣顯示真實狀態。要執行它的任何指令，**包含 `status`**，都要逐個 repo 授權，
而編輯那個檔案會撤銷授權，因為那正是一個指令會被加進去的地方。

一個沒有宣告 port、也還沒被信任的專案，回報的是 *unknown* 而不是 *stopped*。它很可能正在跑，
而**一個對著活著的網站說「掛了」的指示燈，比沒有指示燈更糟**——下一次真的掛掉的時候，看起來一模一樣。

格式在 **[docs/devstack.md](docs/devstack.md)**，附上測試套件會去解析的實際範例，
所以那一頁不可能安靜地變成假的。
...
人在機器前面的時候，輸入條回答「哪一個 session 在等我」。人不在的時候，這個問題一樣值得回答——
而且用的是同一份判讀，所以它有一個網頁。

在**設定 → Remote** 打開，app 就會在 `127.0.0.1` 上提供 session 清單：每一個 session 的狀態、
它所屬專案的圖示、以及照 <kbd>⌘</kbd><kbd>J</kbd> 那樣排版過的逐字稿。桌機是兩欄、配上 app 自己那套快速鍵；
手機是單欄，設計上就是要被加到主畫面的——加了之後它的行為像 app，不像網頁。

要從外面連進來，指向 `cloudflared`：可以用它隨機產生的網址，也可以用你自己的網域。
**連線是往外撥的**，所以沒有 port 要轉發，你的網路上也沒有任何東西在聽。

**兩個開關，因為那是兩種不同的風險。** 讀一個 session 會洩漏 repo 名稱、分支、任務標題；
**寫**進一個 session 則是遠端執行程式碼，因為 Claude Code 會跑 bash。所以送字是第二個開關、
預設關閉，而且是整批授予配對裝置的——這樣要收回的時候是一次收回全部。

沒有配對過的裝置什麼都連不到。配對時那組六位數字**只出現在 Mac 上**，
不在給瀏覽器的回應裡——所以就算有人找到你的網址、也按了配對，他看不到你的螢幕就完成不了。
手機可以改用掃 QR code。而**在配對過任何裝置之前，tunnel 拒絕啟動**：
「從網際網路可達」應該是一個人做的決定，不是改一個設定鍵就會發生的事。

`docs/remote.md` 裡有威脅模型，包含**這一層防不到什麼**；
[docs/api.md](docs/api.md) 則是給腳本或外掛講話的那個介面——所有 session、所有逐字稿、
一條事件串流，而 `curl` 就是唯一的 SDK。
