...
其餘所有功能都是從這一件事推出來的：**既然你不打算看終端機，這條輸入條就得替它把話講完。**

<table>
<tr><td width="50%">

**把 session 讀回來** — <kbd>⌘</kbd><kbd>J</kbd>

排版過的對話：標題、有框線的表格、程式碼。跑完的工具呼叫收成一行，
上面還有一行說它現在正在做什麼。

</td><td width="50%">

**用說的代替打字** — <kbd>⌘</kbd><kbd>L</kbd>

講的時候字就出現。停下來，Whisper 讀同一段錄音把它換掉——
**一句話裡兩種語言**，那是任何即時辨識器都做不到的。
...
不只是哪個任務：repo、分支、未提交數量、進行中的部署、backlog，
還有那個專案自己的圖示與顏色。
...
畫面上是縮圖，送出去的是路徑——那才是 Claude Code 讀得了的東西。
