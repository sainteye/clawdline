...
#
# Making `claims` mandatory adds 19, and the number is half measured and half read. Its two groups
# were run on their own through `CLAWDLINE_TEST_GROUPS` and reported `25 focused checks`, twice:
# once against a mutation restoring the pre-change world — absence warned about rather than refused
# — where the same selection reported `13 of 25 focused checks failed`, and once as delivered. The
# group it replaces, the one that proved an undeclared dispatch was *warned*, executed 6; that 6 is
# counted off the diff rather than off a run, because measuring it would have meant compiling the
# base tree a second time. 25 - 6 = 19. **So this line is arithmetic on top of a measurement, which
# is the weaker of the two kinds of number in this file**: if the exact-tree run reports something
# else, that run is right and this is what has to move. 8,510 -> 8,529.
expected_swift_receipt='8529 checks passed'
