...
enum OrchestratorInventory {
    ...
    static func rows(repository: String, tasks: [Orchestrator.Task],
        ...
        for task in tasks.sorted(by: { $0.id < $1.id }) {
            ...
            case .settled:
                let live = branch.flatMap { branches.known ? branches.heads[$0] : nil }
                if let row = droppableRow(task, exists: exists, merged: merged, head: live,
                                          now: now) {
                    out.append(row)
                }
            }
    ...
    ///
    /// **`head` is the live ref, and the stored one is only its fallback.** The two answer
    /// different questions — ``Orchestrator/inflightRow(_:now:)`` puts it as reporting a delivery
    /// against reporting a memory of one — and only the live ref can say whether this branch has
    /// received a commit since the row was written. Reading the stored value alone cannot change
    /// whether a row is droppable, but it can print `merged_and_clean` for a branch that is
    /// actually `branch_empty`, and `why` is in the digest: a receipt that moves when nothing
    /// decided differently is as bad as one that does not move when something did.
    private static func droppableRow(_ task: Orchestrator.Task, exists: Bool?, merged: Bool?,
                                     head liveHead: String?, now: Date) -> Row? {
        ...
        if !exists {
            ...
            let head = liveHead ?? worktree.head
