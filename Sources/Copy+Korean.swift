...
struct Korean: Copy {
    ...
    let webVoiceSlow = "다시 켜고 난 첫 번째는 모델부터 읽습니다 — 열두 초쯤."
    let webVoiceLimit = "한 번에 녹음할 수 있는 길이는 {n}분입니다. 녹음된 데까지 받아씁니다."
    let webVoiceEmpty = "거기서 아무것도 들리지 않았습니다."
    let webVoiceTooShort = "너무 짧아서 알아들을 수 없었습니다."
    let webVoiceDenied = "마이크가 거부되었습니다. 다시 켜는 곳은 이 사이트에 대한 이 브라우저 자체 설정입니다."
    let webVoiceNoMic = "이 기기에서 마이크를 찾지 못했습니다."
    let webVoiceInUse = "마이크가 사용 중입니다 — 이 기기의 다른 무언가가 쥐고 있습니다."
    let webVoiceInsecure = "마이크에는 보안 연결이 필요합니다. Clawdline을 https로 열면 됩니다."
    let webVoiceUnsupported = "이 브라우저는 녹음할 수 없습니다."
    let webVoiceBusy = "Mac이 이미 두 개를 받아쓰고 있습니다. 잠시 뒤에 다시 해 보세요."
    let webVoiceNoBinary = "Mac에 Whisper가 설치되어 있지 않아, 거기에는 받아쓸 것이 없습니다."
    let webVoiceNoModel = "Mac에 Whisper는 있지만 읽을 모델이 없습니다. ~/.cache/whisper에 ggml 파일 하나면 됩니다."
