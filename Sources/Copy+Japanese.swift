...
struct Japanese: Copy {
    ...
    let settingsOrchestratorMaxHint = "既定は 3 つ。どれもアシスタントが 1 つ入ったターミナルのタブです。"
    ...
    let webVoiceSlow = "再起動後の一回目はモデルの読み込みから始まります — 十二秒ほど。"
    let webVoiceLimit = "一回の録音は {n} 分までです。ここまでの分を文字にします。"
    let webVoiceEmpty = "そこには何も聞こえませんでした。"
    let webVoiceTooShort = "短すぎて聞き取れませんでした。"
    let webVoiceDenied = "マイクが拒否されました。オンに戻せるのは、このブラウザがこのサイトに対して持っている設定です。"
    let webVoiceNoMic = "この端末にマイクが見つかりません。"
    let webVoiceInUse = "マイクが使用中です — この端末の別のものが握っています。"
    let webVoiceInsecure = "マイクには安全な接続が必要です。Clawdline を https で開けば使えます。"
    let webVoiceUnsupported = "このブラウザは録音できません。"
    let webVoiceBusy = "Mac はすでに二件を文字にしています。少ししてからどうぞ。"
