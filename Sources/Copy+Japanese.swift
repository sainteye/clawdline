...
struct Japanese: Copy {
    ...
    let settingsOrchestratorPermissionHint = "子セッションのタブは誰も見ていないので、承認待ちで止まったセッションはタイムアウトまで止まったままです。だから既定は最初ではなく真ん中です。タスクはこれより控えめに要求できますが、これを超えることはできません。"
    let settingsOrchestratorPermissionAsk = "一歩ごとに確認する"
    let settingsOrchestratorPermissionAuto = "何を聞くかは自分で判断させる"
