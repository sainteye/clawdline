...
enum Orchestrator {
    ...
    static func childBrief(for task: Task) -> String {
        ...
        **An expensive compile goes through the machine lease, and there is exactly one slot.**
        ...
        take the slot for themselves, so ordinarily you do nothing; if you start `swiftc` by hand,
        take it first and give it back after:

        ```bash
        curl -s -X POST http://127.0.0.1:\(Config.shared.remotePort)/v1/orchestrator/leases \\
          -H "X-Clawdline-Orchestrator: $(cat ~/.config/clawdline/orchestrator-token)" \\
          -H 'Content-Type: application/json' \\
          -d '{"request_id":"'"$$"'-compile","resource":"heavy_compile","pid":'"$$"',
               "holder":"task \(task.id.prefix(8))","reason":"one focused compile"}'
        ```
