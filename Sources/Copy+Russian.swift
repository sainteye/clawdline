...
struct Russian: Copy {
    ...
    let settingsOrchestratorMaxHint = "По умолчанию три. Каждая — вкладка терминала, в которой работает ассистент."
    ...
    let webVoiceSlow = "Первая после перезапуска сначала грузит модель — секунд двенадцать."
    let webVoiceLimit = "Дольше {n} минут одна запись не идёт. Расшифровываем то, что есть."
    let webVoiceEmpty = "Там ничего не услышали."
    let webVoiceTooShort = "Это было слишком коротко, чтобы что-то расслышать."
    let webVoiceDenied = "Микрофон отклонён. Обратно он включается в настройках, которые сам браузер держит для этого сайта."
    ...
    let webVoiceInsecure = "Микрофону нужно защищённое соединение. Откройте Clawdline по https — и всё заработает."
    let webVoiceUnsupported = "Этот браузер не умеет записывать звук."
    let webVoiceBusy = "Mac уже расшифровывает две такие. Попробуйте через минуту."
