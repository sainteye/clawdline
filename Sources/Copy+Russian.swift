...
struct Russian: Copy {
    ...
    let settingsOrchestratorPermissionHint = "На вкладку дочерней сессии никто не смотрит, поэтому та, что остановилась ради подтверждения, стоит до истечения времени — потому по умолчанию средний пункт, а не первый. Задача может попросить меньше этого, но не больше."
    let settingsOrchestratorPermissionAsk = "Спрашивать на каждом шаге"
    let settingsOrchestratorPermissionAuto = "Пусть сама решает, о чём спросить"
