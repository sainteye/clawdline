...
struct Turkish: Copy {
    ...
    let settingsOrchestratorMaxHint = "Varsayılan üç. Her biri, içinde bir asistanın çalıştığı bir terminal sekmesi."
    ...
    let webVoiceSlow = "Yeniden başlattıktan sonraki ilki önce modeli yüklüyor — on iki saniye kadar."
    let webVoiceLimit = "Bir kayıt en fazla {n} dakika sürer. Elde olan yazıya dökülüyor."
    let webVoiceEmpty = "Orada bir şey duyulmadı."
    let webVoiceTooShort = "Duyulamayacak kadar kısaydı."
    let webVoiceDenied = "Mikrofon reddedildi. Geri açıldığı yer, bu tarayıcının bu site için tuttuğu kendi ayarları."
    let webVoiceNoMic = "Bu cihazda mikrofon bulunamadı."
    let webVoiceInUse = "Mikrofon meşgul — bu cihazda başka bir şey onu tutuyor."
    let webVoiceInsecure = "Mikrofon güvenli bir bağlantı ister. Clawdline'ı https ile aç, çalışır."
    let webVoiceUnsupported = "Bu tarayıcı ses kaydedemiyor."
    let webVoiceBusy = "Mac zaten bunlardan ikisini yazıya döküyor. Birazdan tekrar dene."
