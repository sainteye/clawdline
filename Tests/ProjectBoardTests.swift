...
group("Project Board defaults, mode boundaries, and corrupt stores fail closed") {
  do { let d = BoardTestDriver(name: "large-progress-\(UUID().uuidString)"); d.createProject(); let epic = d.create(type: "epic", title: "Platform program")
    func add(_ item: String, _ title: String, required: Bool = true) -> String { _ = d.send("checklist", ["itemId": item, "title": title, "required": required]); return (d.item(item)["checklist"] as? [[String: Any]])?.last?["id"] as? String ?? "" }; func mark(_ item: String, _ id: String, _ status: String) { _ = d.send("checklist", ["itemId": item, "checklistId": id, "status": status]) }
    let parentRow = add(epic, "Parent summary"); mark(epic, parentRow, "passed")
    let first = d.create(type: "feature", title: "Runtime", parent: epic), second = d.create(type: "feature", title: "Cloud", parent: epic); let firstA = add(first, "Compile"), firstB = add(first, "Smoke"), secondA = add(second, "Deploy"); _ = add(first, "Optional polish", required: false)
    _ = d.send("milestone", ["itemId": first, "title": "Runtime ready"]); let firstMilestone = (d.item(first)["milestones"] as? [[String: Any]])?.last?["id"] as? String ?? ""; _ = d.send("milestone", ["itemId": second, "title": "Cloud ready"])
    mark(first, firstA, "passed"); mark(first, firstB, "doing"); mark(second, secondA, "passed"); _ = d.send("milestone", ["itemId": first, "milestoneId": firstMilestone, "status": "passed"])
    let measurement = boardProgress(d.item(epic))["measurement"] as? [String: Any] ?? [:]; expect("fixed leaf acceptance percentage", measurement["recordedPercent"] as? Int, 60)
    check("parent and optional units are excluded", measurement["excludedParentUnits"] as? Int == 1 && measurement["denominator"] as? Int == 5 && measurement["leafCount"] as? Int == 2)
    let stages = measurement["stages"] as? [String: [String: Any]] ?? [:]; check("four stages stay separate and release starts unknown", stages["planning"]?["percent"] as? Int == 100 && stages["implementation"]?["percent"] as? Int == 80 && stages["verification"]?["percent"] as? Int == 60 && stages["release"]?["percent"] is NSNull); d.store.ingest(task: ["id": "progress-landed", "workItemId": first, "state": "success", "landing": ["state": "landed", "verification_origin": "local_target_branch", "verified_commit": "progress-a", "verified_target_commit": "progress-target"]], projectID: "project-1"); d.store.ingest(task: ["id": "progress-settled", "workItemId": second, "state": "success", "landing": ["state": "nothing_to_land"]], projectID: "project-1"); let afterDelivery = ((boardProgress(d.item(epic))["measurement"] as? [String: Any])?["stages"] as? [String: [String: Any]])?["release"]; check("landed and settled leaves still cannot claim deployment", afterDelivery?["percent"] is NSNull && afterDelivery?["status"] as? String == "insufficient_evidence")
    _ = d.create(type: "epic", title: "Unscoped nested program", parent: epic); let incomplete = boardProgress(d.item(epic))["measurement"] as? [String: Any] ?? [:]; check("unscoped feature or nested Epic refuses precision", incomplete["status"] as? String == "insufficient_scope" && incomplete["recordedPercent"] is NSNull && incomplete["unmeasuredLeafCount"] as? Int == 1) }
  do { let d = BoardTestDriver(name: "large-progress-container-\(UUID().uuidString)"); d.createProject(); let program = d.create(type: "epic", title: "Program"), phase = d.create(type: "epic", title: "Phase"), leaf = d.create(type: "feature", title: "Leaf", parent: phase); _ = d.send("link", ["itemId": program, "kind": "related", "targetId": phase, "label": "Phase"]); _ = d.send("link", ["itemId": program, "kind": "related", "targetId": leaf, "label": "Leaf"])
    _ = d.send("checklist", ["itemId": phase, "title": "Phase summary", "required": true]); let phaseRow = (d.item(phase)["checklist"] as? [[String: Any]])?.last?["id"] as? String ?? ""; _ = d.send("checklist", ["itemId": phase, "checklistId": phaseRow, "status": "passed"]); _ = d.send("checklist", ["itemId": leaf, "title": "Leaf acceptance", "required": true]); let measurement = boardProgress(d.item(program))["measurement"] as? [String: Any] ?? [:]; check("related container summaries never double-count their leaf acceptance", measurement["status"] as? String == "available" && measurement["leafCount"] as? Int == 1 && measurement["denominator"] as? Int == 1 && measurement["recordedPercent"] as? Int == 0) }
    ...
    expect("the public Board wire remains compatible while storage migrates", board?["schemaVersion"] as? Int, 1)
    expect("the current entitlement is free", (board?["entitlement"] as? [String: Any])?["state"] as? String,
           "free_preview")

    d.createProject()
    let item = d.create(); do { let c = BoardTestDriver(name: "catalog-\(UUID().uuidString)"); c.createProject(); let child = c.create(), parent = c.create(type: "epic", title: "Human outcome"), technical = c.create(type: "task", title: "Review detail"); let catalogEntry: [String: Any] = ["itemId": technical, "expectedScopeRevision": 0, "audience": "agent", "role": "execution_record", "parentId": NSNull(), "outcome": "technical_detail", "reason": "Exact review receipt retained"], childEntry: [String: Any] = ["itemId": child, "expectedScopeRevision": 0, "audience": "human", "role": "subtask", "parentId": parent, "outcome": "actionable", "reason": "Bounded child of the exact outcome"]; func reconcile(_ audit: String, _ entries: [[String: Any]], trusted: Bool = false) -> ProjectBoardStore.Reply { c.send("reconcile_catalog", ["projectId": "project-1", "auditId": audit, "items": entries], trusted: trusted) }; expect("catalog reconciliation is machine-only", boardError(reconcile("audit-1", [catalogEntry])), "trusted_reconciliation_required"); expect("exact catalog reconciliation is atomic command success", reconcile("audit-1", [catalogEntry, childEntry], trusted: true).status, 200); let technicalView = (c.item(technical)["listSummary"] as? [String: Any])?["view"] as? [String: Any], childView = (c.item(child)["listSummary"] as? [String: Any])?["view"] as? [String: Any]; check("technical records leave the human board without losing their item", technicalView?["audience"] as? String == "agent" && technicalView?["role"] as? String == "execution_record"); check("curated subtasks retain their exact display parent", childView?["role"] as? String == "subtask" && childView?["parentId"] as? String == parent); let cycle: [String: Any] = ["itemId": parent, "expectedScopeRevision": 0, "audience": "human", "role": "subtask", "parentId": child, "outcome": "actionable", "reason": "would create a cycle"]; expect("catalog reconciliation rejects a recursive subtask hierarchy", boardError(reconcile("audit-cycle", [cycle], trusted: true)), "catalog_parent_cycle"); let before = c.revision, stale: [String: Any] = ["itemId": technical, "expectedScopeRevision": 99, "audience": "archive", "role": "provenance_record", "outcome": "historical_gap", "reason": "stale"]; expect("one stale scope rejects the whole catalog batch", boardError(reconcile("audit-stale", [stale], trusted: true)), "catalog_scope_conflict"); expect("a refused catalog batch does not advance revision", c.revision, before); let reloaded = ProjectBoardStore(url: c.file), reloadedView = ((reloaded.snapshot(item: technical)["board"] as? [String: Any])?["item"] as? [String: Any])?["listSummary"] as? [String: Any]; expect("catalog disposition survives restart", (reloadedView?["view"] as? [String: Any])?["audience"] as? String, "agent"); var corrupt = try! JSONSerialization.jsonObject(with: Data(contentsOf: c.file)) as! [String: Any], storedItems = corrupt["items"] as! [[String: Any]], parentRow = storedItems.first(where: { $0["id"] as? String == parent })!, invalidDisposition = storedItems.first(where: { $0["id"] as? String == child })!["catalogDisposition"] as! [String: Any]; invalidDisposition["parentId"] = child; parentRow["catalogDisposition"] = invalidDisposition; storedItems[storedItems.firstIndex(where: { $0["id"] as? String == parent })!] = parentRow; corrupt["items"] = storedItems; try! JSONSerialization.data(withJSONObject: corrupt).write(to: c.file, options: .atomic); let corruptCatalog = ProjectBoardStore(url: c.file); expect("persisted catalog cycles fail closed on restart", ((corruptCatalog.snapshot()["board"] as? [String: Any])?["error"] as? [String: Any])?["code"] as? String, "board_store_corrupt") }
