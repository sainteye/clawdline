...
group("board HTTP authority cannot be supplied by command content") {
    ...
    expect("machine auth preserves baseline local control", call("{}", machine: true)?.status, 400); let catalogHTTP = BoardTestDriver(name: "catalog-http-\(UUID().uuidString)"); catalogHTTP.createProject(); let catalogHTTPItem = catalogHTTP.create(title: "Technical receipt"), catalogHTTPBody: [String: Any] = ["operation": "reconcile_catalog", "requestId": "catalog-http", "expectedRevision": catalogHTTP.revision, "projectId": "project-1", "auditId": "catalog-http-audit", "items": [["itemId": catalogHTTPItem, "expectedScopeRevision": 0, "audience": "agent", "role": "execution_record", "outcome": "technical_detail", "reason": "HTTP authority proof"]]], catalogHTTPJSON = String(data: try! JSONSerialization.data(withJSONObject: catalogHTTPBody), encoding: .utf8)!; Config.shared.remoteWrite = true; ProjectBoardHTTP.configureStoreForTesting(catalogHTTP.store); expect("paired admin cannot forge catalog reconciliation authority from an otherwise valid command", call(catalogHTTPJSON, caps: [.read, .send, .admin])?.status, 403); expect("local machine route admits exact catalog reconciliation", call(catalogHTTPJSON, machine: true)?.status, 200); ProjectBoardHTTP.configureStoreForTesting(nil); Config.shared.remoteWrite = false
