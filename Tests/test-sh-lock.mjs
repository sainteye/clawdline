...
// Full completion is now self-describing: totals may change, while uniqueness and roster shape
// remain strict. Drive test.sh's read-only verifier so a source-controlled count cannot creep back
// in under another name.
{
    const work = mkdtempSync(join(tmpdir(), "completion-receipt-"));
    try {
        let sequence = 0;
        const verify = (body) => {
            const log = join(work, `run-${++sequence}.log`);
            writeFileSync(log, body);
            const result = spawnSync("/bin/bash", ["test.sh", "--verify-completion-receipts", log],
                { cwd: new URL("../", import.meta.url), encoding: "utf8", env: process.env });
            return { status: result.status, output: `${result.stdout}${result.stderr}` };
        };
        const roster = /^cloud_suite_roster='([^']+)'$/m.exec(script)?.[1].split(",") ?? [];
        const cloud = `CLAWDLINE_CLOUD_TESTS_COMPLETE v=1 suite_count=${roster.length} suites=`
            + roster.map((name, index) => `${name}:${index + 1}`).join(",");
        check("a self-consistent completion receipt accepts any observed Swift and Cloud counts",
            verify(`42 checks passed\n${cloud}\n`).status === 0
              && verify(`9001 checks passed\n${cloud.replace(`${roster[0]}:1`, `${roster[0]}:4`)}\n`).status === 0);
        check("a missing or duplicate completion receipt is refused",
            verify("42 checks passed\n").status === 125
              && verify(`42 checks passed\n42 checks passed\n${cloud}\n`).status === 125);
        check("Cloud suite-count mismatch and duplicate names are refused",
            verify(`42 checks passed\n${cloud.replace(`suite_count=${roster.length}`, `suite_count=${roster.length + 1}`)}\n`).status === 125
              && verify(`42 checks passed\n${cloud.replace(`${roster[1]}:2`, `${roster[0]}:2`)}\n`).status === 125);
        check("no source-controlled completion total or reseal door remains",
            !/^expected_(?:swift|cloud)_receipt=/m.test(script)
              && !/^expected_swift_receipt_witness=/m.test(script)
              && !/apply-test-receipt-seal/.test(script));
    } finally {
