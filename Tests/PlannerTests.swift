...
group("every word the page can draw is a word the page is sent") {
    ...
    let derived: Set<String> = ["webOrderNewest", "webOrderOldest",  // t.outputOrder(newestFirst:)
                                ...
                                "webImageExpired", "webImageUnknown", "webImagePreview",
                                "webImageClose", "webImageTooLarge", "webImageUnavailable"]
    ...
    let cloudDefaults = stringsSource.components(separatedBy: "// clawdline:cloud-default-copy:begin")
        .dropFirst().first?.components(separatedBy: "// clawdline:cloud-default-copy:end").first ?? ""
    let cloudDefaultNames = Set(cloudDefaults.split(separator: "\n").compactMap { line -> String? in
        let words = line.trimmingCharacters(in: .whitespaces).split(separator: " ")
        guard words.count >= 2, words[0] == "var" else { return nil }
        return String(words[1].dropLast())
    })
    check("the temporarily-English Cloud batch is explicit and bounded",
          cloudDefaultNames.count == 45 && cloudDefaultNames.contains("webFailUnknown")
              && cloudDefaultNames.contains("webCloudStatusMac"),
          "expected the named 45-key batch, found \(cloudDefaultNames.count)")
