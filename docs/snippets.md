...
`api.md`, which was written from
`Snippets.swift` and the route cases rather than from here.
...
- Typed refusals, each with a code of its own so a sheet can say which it hit:
  ...
  `api.md` with what each one means.
...
`docs/api.md` carries one section for the set, beside
