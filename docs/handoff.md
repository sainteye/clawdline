...
This is the sister page to `docs/orchestrator.md`, and the two describe opposite
...
`orchestrator.md` follows `parent_task` and `root.session_id` links, a
...
2. **Name this session after the job.** Once the document has said what the job is, the receiver
   ...
   `POST /v1/sessions/:id/title` — the device token from
   ...
   title, it is rung 1 of `TargetSession.preferredDisplayLabel`, and it
   ...
   session running its own tool call is *working*, which `api.md`
   ...
   by exactly the rule `api.md` gives — the newer of the two
...
{"ok":true,"handoff":{"id":"c6000020-0000-4000-8000-000000000020","state":"opening","projectDir":"/Users/you/code/clawdline","assistant":"codex","dir":"/tmp/.clawdline/handoffs/c6000020-0000-4000-8000-000000000020","opened":{"terminalId":"9A1F…","backend":"iterm"}}}
```
...
- **A terminal id is reusable, so the label is bound to more than a terminal id.** Once a reading has matched
  ...
  `TargetSession.preferredDisplayLabel` and does not move: a name a
...
| `assistant` | optional. `claude` or `codex`. Absent is `claude`, the same default `POST /v1/places/:id/start` has |
...
with `admin`, not over a tunnel. The reasoning is `the credential table`
...
more**, exactly as `the dispatch brake` is — because a loop that opens tabs is
...
`POST /v1/sessions/:id/send` types into it,
`POST /v1/sessions/:id/end` closes it. A handoff is an event and not
...
`docs/orchestrator.md`. Every route in full is `docs/api.md`. The trial
