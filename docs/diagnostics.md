...
ceiling (16 MiB since private Cloud service decision D17), and over it the panel says `browser · report_too_large`
...
| `machine_pairing_lookup` | this browser's pairing for that machine, from memory or the store | `machine_key_incomplete`; `extractable_key`; a raw store error | none (machine-local repair); encryption; none |
...
| `legacy_binding` | a machine with no pairing is bound to the pin that just opened it | `machine_key_incomplete`, `machine_pairing_required` | none (machine-local repair); pairing |
