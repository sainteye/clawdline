...
與 ciphertext；`CloudEnvelope.swift`:174–235 在 host seal/open；
`CloudTransport.swift`:875–896 解密後交 inbound command。
...
[docs/cloud.md](../cloud.md):9–11 指向 private Cloud service 的 `PROTOCOL.md` 作為現有
...
- 固定 source 使用 CryptoKit；`CloudEnvelope.swift`:49、139–147、174–235 有 12-byte AES-GCM nonce、簽章輸入與 seal/open；`CloudKeys.swift`:3–6、456–483 有 Ed25519／content key 與 Keychain storage seam。這些是 current bytes，不能由此改寫未檢查的 private authority。
- `CloudTransport.swift`:207–249 有 socket／URLSession seam，`:875–896` 使用 pinned sender key 與 sequence gate；`CloudAppBridge.swift`:199–238 仍路由回 RemoteServer。
