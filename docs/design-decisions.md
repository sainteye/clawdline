...
| D19 | **target 只看紀錄上的**：第一次 landing 時由 root 決定並記下；之前是「未定」。inventory 判斷 merged 用紀錄上的 target，**未定就是未知，不拿主 repo 的 `HEAD` 代替**。例外（2026-09-25）：broker 偵測到已合併時，只有在「含有 delivery head、扣掉 `clawdline/task/*` 與 linked worktree 正 checkout 的本機 branch」恰好剩一個、且它就是主 checkout 的 branch 時才自己命名（`landing_detect.go`） | 兩份文件一致（broker-design #32：「Which branch a record should have named is a person's decision」）；B1 兩處違反 | 無 | `dispatch.go:612` 開 pending 時沒有 target；`inventory.go:239` 用 `"HEAD"` | W3 |
...
| D69 | **An unread direct to-do can be sent again once it is two minutes old** (`app.DirectTodoResendAfter`). Inside the window a second Send is refused as `todo_awaiting_read`; past it, or once the Session has read its queue, Send is offered again. In the row, Send is the one emphasized action and Delete is a quiet trash can at the far end. | The earlier rule refused any second delivery until `read_at`, and a Session that never read its queue left the row with no Send at all: one to-do waited from 09:59 to 12:40 on 2026-09-25 with only Delete offered. The window keeps what the rule was for, a double tap typing the words twice. | D67's "sent again only after read" | This change |
