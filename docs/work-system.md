...
**CM-2** session 不能自己動你的看板：用 orchestrator token 寫 `/v1/work/*` 一律 403 `session_cannot_decide`，除非帶 `{"via":{"run":"…"}}` 代轉你說的話，actor 記 `user_via_session:<run>`。〔部分：run 只驗格式，沒有任何路由發 run（缺口 2）〕
...
**PT-3** 規則自己提：sweep 對「派工帶了 `work_id`、但還沒有項目」的工作線提議，只進待確認、不在對話裡問。〔部分：沒帶 `work_id` 的派工永遠不會被規則提議（缺口 1）〕
...
1. **派工不帶 `work_id`，看板收不到事實**（BD-4、BL-6、PT-3、TD-7）。broker 只驗格式（`app/orchestrator/draft.go`
   `admitWorkID`），不驗項目存在；guide 只寫「選填」。今天 0 筆帶。要不要讓派工必帶、或由 broker 依規則綁定（D36
   的原文），由使用者決定；在那之前，看板上只會有人手動放上去的項目。
2. **`via.run` 沒有發放者**（CM-2、U4）。`transport/http/work.go` `runID` 只驗格式；run 原本來自退役的 workflow
   信封，現在沒有任何路由發。guide §10 叫 session 不要代轉，但今天 Backlog 的 10 項全是代轉。補一個發 run 的來源，
   或改掉 U4，由使用者決定。
