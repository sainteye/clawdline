...
| Machine usage dashboard (CPU, memory, each session's share) | `/proc/stat`, `/proc/meminfo`, `/proc/<pid>/stat`, `/proc/pressure` | ✅ `machineusage/read_linux.go` | ✅ | ❌ `read_other.go` returns `ErrUnsupported` (macOS needs `host_statistics` + `proc_pidinfo`, Windows the PDH counters) | `GET /v1/machine/usage` answers 501 `machine_usage_unsupported`; the dashboard says the system has no reader yet, not that the machine is idle | M |
