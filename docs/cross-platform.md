...
| Machine usage dashboard (CPU, memory, each session's share) | Linux: `/proc`; macOS: `ps -axo pid,ppid,rss,time,comm`, `vm_stat`, `hw.memsize`, `vm.swapusage`, `vm.loadavg` | ✅ `machineusage/read_linux.go`, `read_darwin.go` (no cgo: a Mac's CPU is its processes' CPU time over cores × interval, `kernel_task` included; no pressure-stall reading and no per-process swap) | ✅ | ❌ `read_other.go` returns `ErrUnsupported` (Windows needs the PDH counters and a toolhelp snapshot) | `GET /v1/machine/usage` answers 501 `machine_usage_unsupported`; the dashboard says the system has no reader yet, not that the machine is idle | M |
