...
| N46 | Board items a Session claims on one of the person's messages | **5 claims** per run, active or released, registered as `run.claimed_items`, mirroring N39; the call under the 96 KiB work request body | The sixth claim on one run is refused `run_claims_exhausted`; no assignment, owner or history row is written. | The sender receives a typed refusal; the row is present in `/v1/diagnostics.capacity`. | No | ✓ |
