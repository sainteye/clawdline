...
| N47 | Board items a Session claims on one of the person's messages | **5 claims** per run, active or released, registered as `run.claimed_items`, mirroring N39; the call under the 96 KiB work request body | The sixth claim on one run is refused `run_claims_exhausted`; no assignment, owner or history row is written. | The sender receives a typed refusal; the row is present in `/v1/diagnostics.capacity`. | No | ✓ |
...
| N49 | Sessions a reboot took away (`app.SessionRestore`, `store.RecordBoot`, `/v1/sessions/restorable`, docs/session-restore.md) | **200 conversations** recorded per boot, **2 boots** kept (this one and the one before), **20 conversations** per restore, and a recorded `last_seen` at most **300 seconds** stale while the open set is unchanged, registered as `sessions.restore_rows`, `sessions.restore_boots`, `sessions.restore_batch` and `sessions.restore_seen_age` (`restoreRowsLimit` … `restoreSeenLimit` in `app/session_restore.go`) | Past 200 the conversations that moved longest ago are not recorded and counted as evicted. A third boot deletes the oldest boot and its rows at the next write. A restore naming more than 20 is refused whole with `restore_batch_too_large` and opens nothing. An unchanged set is rewritten only once its `last_seen` is 300 seconds old. | `/v1/diagnostics.capacity` carries all four rows, the evicted count on `sessions.restore_rows`; the sender of an oversized restore receives the typed refusal | Rows past 200 and boots older than the previous one cannot be offered back; nothing a person wrote is lost | ✓ |
