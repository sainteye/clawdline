...
`api.md` for the request contract.

Before that attestation, the same route family now exposes one exact Board-accountability read.
It joins the Session's direct Board relations with descendant tasks' explicit `workItemId` values
and exact child-Session relations,
pins every item scope and produces the only `audit_id` accepted when that set is non-empty. This
does not let closure declare a card complete: the inventory passes only when existing Board and
broker facts already show a settled result, accepted transfer, settled agent detail, or a current
assistant delivery with enough next-action text for takeover. Open spans, live children and
pending transfers and missing referenced item identities fail closed. Its digest is part of the
opaque closeability version, so a Board change after attestation invalidates the final close CAS.
Board OFF preserves retained history without adding this Board-only gate.
