...
| Codex | `codex app-server`'s JSON-RPC — `initialize`, `model/list`, `thread/read` and `thread/name/set`. The one dependency here with a real contract: it has a generator, `codex app-server generate-json-schema` | `CodexNaming.swift` | not known to have a floor | A Codex session opened to be handed work never gets the name it was opened under, and the panel lists it as an untitled thread |
