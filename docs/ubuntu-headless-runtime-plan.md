...
`/tmp/.clawdline/aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa/artifacts/W2_3_REVIEW.md`; the boundary and
what remains are recorded in `architecture-refactor.md`.
...
Core/Application/Mac target graph" block **used to** check the symlinks, the candidate-manifest
membership and the SwiftPM dependency edges themselves (via `swift package describe --type json`)
rather than
