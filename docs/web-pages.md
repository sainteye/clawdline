...
app watches are usually tmux panes, and a pane id is `%14` — `Sources/Tmux.swift` calls `%12`
"stable for the life of the pane". Written raw, the address read `/#session=%14`, and the reader
...
looks like: `%14` decodes to U+0014 and a `1`, a control character no session id contains. A link
written since the encoding decodes to a real id instead — `%2514` to `%14` — and there the raw
...
`GET /v1/orchestrator/usage/verification-ledger`
