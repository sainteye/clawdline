...
> 目標：舊 app 退役。它現在還扮演這台機器的 broker——每一個 agent session 的派工、落地、訊息、收據都走它的
> `/v1/orchestrator/*`（`Sources/Orchestrator.swift` 10,745 行）。這一波只做「一個 root 從派工到收工」那條路，
> 做完就能自己跑一次完整迴圈。
...
以及對 7717 的 `/inflight`（只用本任務自己的 secret）。
