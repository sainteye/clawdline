...
Working examples of every file are in `examples/`, and the test suite parses them,
...
`/code/sample-project` covers `/code/sample-project/frontend`, and `/code/sample-project/backend` can still have its own.
...
```jsonc
{
  ...
  "url": "https://github.com/you/sample-project/actions/runs/31981652530"
}
```
...
```jsonc
{
  ...
  "tree": "/Users/you/code/sample-project"           // optional, which checkout it is running in
}
```
...
Working example: `examples/run--Users-you-code-sample-project.json`.
...
```jsonc
{
  ...
  "artifact": "/Users/you/code/sample-project/backend/artifacts/backlog.html"
}
```
...
```jsonc
{
  ...
  "artifact": "/Users/you/code/sample-project/backend/artifacts/launch-milestone.html"
}
```
...
`/Users/you/code/sample-project` → `-Users-you-code-sample-project`. The prefix ends in a dash and the path begins
with one, so the real file name has two: `health--Users-you-code-sample-project.json`.
...
gh run list --limit 1 --json databaseId,status,url,workflowName \
  ...
  > ~/.claude/statusline-cache/ghrun-you-sample-project.json
```
