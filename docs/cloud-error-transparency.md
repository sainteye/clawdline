...
| 2 | `relay` | private Cloud service relay | token 過期、被撤銷或被取代；簽章不符；容量；Mac 不在線（`machine_offline`） |
...
```
cloud: refusal layer=mac_ledger code=command_clock_uncertain sender=web_f052dcb8-… seq=1234 request=3bff2f2b-… type=voice session=%12 status=503 reply=published detail.reason=stability_period_incomplete
```
...
`relayed` 只證明 relay 把信封寫進了 Mac 的 socket。private Cloud service 的 `contracts/cloud/v1/vocabularies.json` 的
...
這一節的程式在 private Cloud service repository，這一輪納入（§8 決定 3）；要不要部署仍是使用者的決定。
...
| L2 | `command_clock_uncertain`：時鐘守衛的原因在 `CloudBridgeLifecycle.swift:147-150` 被丟掉。**這個窗口本身由 task-aaaaaaaa 修**，這裡只負責讓它看得見 | 同 L1 | `detail.reason`（例如 `stability_period_incomplete`）加上 `clears_in_ms`；畫面顯示「Mac 正在確認時間，約 N 秒後再試」 | T-L2 |
...
2. **transport 不准同步 throw。**
   ...
   - task-bbbbbbbb 正在修 `_place` 的同步 throw。這條規則要等它落地之後，再把它的做法推廣到全部方法，
...
每一條都要**先在修正前的樹上跑紅，再在修正後跑綠**（`verification-workflow.md` 的 guard 規則）。
...
2. **多分頁撞號要修掉。** Mac 的 replay 檢查改成滑動窗口：記住最高值以下 1,024 個已經收過的 seq，
   ...
   - 沒選：只讓它看得見。
3. **這一輪包含 relay 的 R1–R5。** 由 child 在 private Cloud service repository 實作並跑 workerd 測試；
   **部署 relay 之前另外問使用者。**
...
- **先等兩件落地：** task-bbbbbbbb（`cloud-client.js`、`start.js`、`i18n.js`）與 task-aaaaaaaa
  （`CloudBridgeLifecycle.swift`、`CloudClock.swift`、`docs/cloud.md`）。在那之前，這條線只動**新檔**與不重疊的檔案。
- **切成四件，每件都能獨立落地：**
  1. **前端錯誤呈現**：B1–B7、B9、`failure-text.js`、transport 契約守衛。等 task-bbbbbbbb。
  2. **Mac 錯誤契約與狀態快照**：M1–M5、P1–P2、L1–L4、F1、G1–G2、`CloudStatus.swift`、`cloud-status.json`。
     等 task-aaaaaaaa；會動到 `CloudTransport.swift`、`CloudAppBridge.swift`、`CloudCommandLedger.swift`。
  3. **手機的狀態 sheet**：`cloud.status` 讀取、notice 的呈現、Cloud 上的 `diagnostics.report`。依賴 1 與 2。
  4. **Relay**：R1–R5，在 private Cloud service repository。不依賴另外兩件，可以先做。
...
- Mac：用 `DiagnosticReport.save` 寫同一組固定檔案，`written_by` 是信封的 sender。
  ...
  `layer` 為 `mac_route`。
- 瀏覽器：只有已宣告 `cloud_status.v >= 1` 的 Mac 才收到這個新指令；舊 Mac 立即回本機的版本不相容提示。送出前先檢查序列化後的大小，超過 private Cloud service decision D17 的 relay per-class cap 或 `DiagnosticReport.maxBytes` 中較小的那個，就在本機以 `browser · report_too_large` 失敗，不送出。
