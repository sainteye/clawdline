...
| Board／Session reads | [RemoteServer.swift](../Sources/RemoteServer.swift) 的 authenticated route switch；[ProjectBoardHTTP.swift](../Sources/ProjectBoardHTTP.swift) 分開 capability、read selectors 與 write gate；[CloudAppBridge.swift](../Sources/CloudAppBridge.swift) 的 `CloudHeadlessRead` 是 closed typed vocabulary | 保持 project／Session scope、authorised read 與無權限拒絕；不能由 read 授予 dispatch/write，不能改 Board consent 或 lifecycle | W4-3；本任務只描述 seam，不編修 Board implementation／tests／原有 docs。W2-2 另加 Project worktree lifecycle read model（[project-worktrees.md](project-worktrees.md)）：read／refresh 為 read 能力，cleanup preview／apply 只接受 machine token，Cloud 詞彙沒有 cleanup |
