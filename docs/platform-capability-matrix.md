...
| terminal lifecycle | 同上；send-keys 成功只表示送達 tty，不等於 shell／agent 已執行；[RemoteServer.swift](../Sources/RemoteServer.swift):3350–3355 擁有 mutation admission seam | create、send、observe、interrupt、resize（支持時）、close、enumerate 的真實 tmux contract；共享一個 admission owner，完整區分 accepted/executed/delivered/observed/acknowledged | W2-1/W4-1；不因 Linux adapter 再建第二條 terminal lane |
