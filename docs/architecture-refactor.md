...
|---|---:|---|
| ordered groups | 534 | `Tests/TestGroupManifest.swift`, counted by the guard |
| ordered runners | 33 | `Tests/main.swift`, counted by the guard |
| suite files | 46 | `Tests/*Tests.swift`, counted by the guard |
| Swift checks | 8,986 | `expected_swift_receipt` in `test.sh`, set from a run |
| `Orchestrator.swift` ceiling | 10,604 | the ratchet in `tools/check-architecture-boundaries.sh` |
| `RemoteServer.swift` ceiling | 5,676 | the receipt in `tools/check-architecture-boundaries.sh` |
