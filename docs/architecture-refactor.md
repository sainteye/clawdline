...
| Swift checks | 8,632 | `expected_swift_receipt` in `test.sh`, set from a run |
| `Orchestrator.swift` ceiling | 10,628 | the ratchet in `tools/check-architecture-boundaries.sh` |
| `RemoteServer.swift` ceiling | 5,537 | the receipt in `tools/check-architecture-boundaries.sh` |
