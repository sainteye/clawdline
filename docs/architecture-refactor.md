...
|---|---:|---|
| ordered groups | 567 | `Tests/TestGroupManifest.swift`, counted by the guard |
| ordered runners | 39 | `Tests/main.swift`, counted by the guard |
| suite files | 52 | `Tests/*Tests.swift`, counted by the guard |
| Swift checks | 10,112 | `expected_swift_receipt` in `test.sh`, set from a run |
| `Orchestrator.swift` ceiling | 10,826 | the ratchet in `tools/check-architecture-boundaries.sh` |
