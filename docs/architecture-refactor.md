...
| Swift checks | 9,582 | `expected_swift_receipt` in `test.sh`, set from a run |
| `Orchestrator.swift` ceiling | 10,742 | the ratchet in `tools/check-architecture-boundaries.sh` |
| `RemoteServer.swift` ceiling | 5,834 | the receipt in `tools/check-architecture-boundaries.sh` |
