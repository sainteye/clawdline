...
相關：`docs/api.md`（路由與授權）、`docs/backlog.yaml`。
...
| **`RemoteServer.swift` 的行數天花板** | 不動 | 不動 | ~~**要動**：`tools/check-architecture-boundaries.sh:184` `remote_server_ceiling=5570`，只剩 **5 行**餘裕，`:488` 還有反向守衛~~ **2026-09-16 更正：那個守衛檔案已經不在樹裡，這個天花板不存在。**不要照著它設計。 | 
...
~~`tools/check-architecture-boundaries.sh`（`:184`–190、`:488`）~~（已移除）、`tools/check-web-strings.py`、
