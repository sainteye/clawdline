...
//
// `-F pfn` is the machine-readable form — a `p<pid>` line, then an
// `f<descriptor>` and `n<name>` pair per entry — which is parsed instead of
// columns for the same reason Scan does not count `ps` columns: a
// human-readable table changes width. The descriptor is what distinguishes
// the process's `cwd` from an ordinary open file without guessing from order.
func systemOpenFiles(ctx context.Context, pids []int) (map[int][]string, map[int]string, bool) {
	if len(pids) == 0 {
		return map[int][]string{}, map[int]string{}, true
	}
	...
	// name lookups this has no use for, and `-d` keeps the text and root entries
	// out. `cwd` deliberately stays: before the first Codex message there is no
	// rollout yet, and this is the only row already read that knows which
	// project the process was opened in.
	//
	...
	cmd := exec.CommandContext(ctx, "/usr/sbin/lsof", "-w", "-n", "-P", "-a", "-F", "pfn", "-d", "^txt,^rtd", "-p", strings.Join(list, ","))
	...
	files, cwds := parseLsof(string(out))
	return files, cwds, true
}

// parseLsof reads lsof's `-F pfn` output. `p<pid>` opens a process block,
// `f<descriptor>` opens one entry, and its following `n<name>` names it.
func parseLsof(out string) (map[int][]string, map[int]string) {
