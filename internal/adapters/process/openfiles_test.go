...
func TestUnreadableIsNotTheSameAnswerAsNoRecord(t *testing.T) {
	unread, unreadBinding := codexConversation(nil, false)
	absent, absentBinding := codexConversation(nil, true)
...
func TestTheOpenRolloutNamesTheConversation(t *testing.T) {
	id, binding := codexConversation([]string{
...
func TestTwoOpenRolloutsAreAmbiguousRatherThanAGuess(t *testing.T) {
	id, binding := codexConversation([]string{rolloutA, rolloutB}, true)
	if id != "" {
	...
	// thread, not two.
	if id, binding := codexConversation([]string{rolloutA, rolloutA}, true); id != idA || binding != session.BindingOpenFile {
...
func TestOnlyUnnamedCodexRowsAreLookedUp(t *testing.T) {
	asked := [][]int{}
	p := &PS{Open: func(ctx context.Context, pids []int) (map[int][]string, bool) {
...
func TestATableThatCouldNotBeReadLeavesEveryRowUnreadable(t *testing.T) {
	p := &PS{Open: func(ctx context.Context, pids []int) (map[int][]string, bool) { return nil, false }}
